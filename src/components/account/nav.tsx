...
export function AccountNav({
  ...
  const dashboard = useSiteText('account.nav.dashboard', 'پیشخوان')
  const orders = useSiteText('account.nav.orders', 'سفارش‌ها')
  const payLater = useSiteText('account.nav.payLater', 'پرداخت بعدی')
  const wishlist = useSiteText('account.nav.wishlist', 'علاقه‌مندی‌ها')
  const addresses = useSiteText('account.nav.addresses', 'نشانی‌ها')
  const reviews = useSiteText('account.nav.reviews', 'دیدگاه‌ها')
  const profile = useSiteText('account.nav.profile', 'اطلاعات حساب')
  const aria = useSiteText('account.nav.aria', 'ناوبری حساب کاربری')
